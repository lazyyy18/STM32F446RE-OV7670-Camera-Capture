...
typedef struct __attribute__((packed))
{
    ...
    uint16_t width;            // 160
    uint16_t height;           // 120
    uint16_t format;           // 1 = RGB565
    uint16_t bytes_per_pixel;  // 2
    uint32_t payload_len;      // 38400
...
static uint8_t OV7670_Init_RGB565_QQVGA_ColorBar(void);
static HAL_StatusTypeDef Camera_StartSnapshot(void);

static uint32_t Camera_Checksum8(const uint8_t *data, uint32_t len);
static void Camera_SendFrame_UART(void);
...
static const uint8_t ov7670_yuv422_qqvga_normal[][2] = {
		/* Reset sensor */
		{REG_COM7, 0x80},
		{OV7670_REG_DELAY, 10},
...
static uint8_t OV7670_Init_RGB565_QQVGA_ColorBar(void)
{
    ...
    if (!OV7670_WriteArray(ov7670_yuv422_qqvga_normal)) {
...
static void Camera_SendFrame_UART(void)
{
    const uint8_t *payload = (const uint8_t *)frame_buffer;

    CameraFrameHeader_t header = {
        .magic = {FRAME_MAGIC0, FRAME_MAGIC1, FRAME_MAGIC2, FRAME_MAGIC3},
        .width = CAM_W,
        .height = CAM_H,
        .format = FRAME_FORMAT_RGB565,
        .bytes_per_pixel = CAM_BYTES_PER_PIXEL,
        .payload_len = FRAME_BYTES,
        .checksum = Camera_Checksum8(payload, FRAME_BYTES)
    };
...
int main(void)
{
  ...
  if (!OV7670_Init_RGB565_QQVGA_ColorBar()) {
  ...
  printf("Sending final frame...\r\n");
  Camera_SendFrame_UART();
  printf("Frame sent over UART\r\n");
  /* USER CODE END 2 */
